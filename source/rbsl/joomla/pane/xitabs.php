 ...
 */
class JPaneRb_tabs extends JPane
{
	/**
	 * Constructor
	 *
	...
	function endPane()
	{
		...
		foreach($this->_data as $id => $data){
			...
			$header .= '<li><a href="#xitabs-'.$id.'">'.$title.'</a></li>';
			$html   .=  '<div id="xitabs-'.$id.'"><p>'.$content.'</p></div>';
		}
		
		return '<ul>'.$header.'</ul>'. $html.'</div>';
	}
	...
	{
		$this->_data[$id]['title'] = $text;
		$this->_data[$id]['content'] = '';
		return '';
		//return '<dt id="'.$id.'"><span>'.$text.'</span></dt><dd>';
	}

	function appendtoPanel($data, $id)
	{
		$this->_data[$id]['content'] .= $data;
	}
	/**
	 * Ends a tab page
	 */
